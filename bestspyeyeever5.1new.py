...
def main(target_ip):
    ...
    actions = [
        ...
        ("TestSSL", run_testssl),
        ("SQLMap", run_sqlmap),
        ("SMB Exploit", exploit_smb),
        ("Ransomware", deploy_ransomware),
        ("SpyEye Botnet", deploy_spyeyebotnet),
        ("Malware Deployment", deploy_malware),
        ("Spoofing Attack", perform_spoofing_attack),
        ("Traffic Sniffing", sniff_traffic),
        ("IP Camera Hack & Monitor", hack_and_monitor_ip_camera),
    ]

    for action_name, action_func in actions:
        action_func(target_ip)
